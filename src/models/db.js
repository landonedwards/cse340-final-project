...
// try to get certificate from environment variable first (Render deployment)
let caCert = process.env.CA_CERT;

// if it's not in environment, try reading local file (local development)
if (!caCert) {
    try {
        // read the CA certificate content
        caCert = fs.readFileSync(path.join(__dirname, '../../bin', 'byuicse-psql-cert.pem'));
    } catch (error) {
        console.warn('Warning: CA_CERT environment variable not set and local cert file not found.');
    }
}
